#!/usr/bin/env python3
"""Generate AI agent self-portraits via SkillBoss API Hub (stdlib only)."""
...
def skillboss_generate(api_key: str, prompt: str) -> bytes:
    payload = {
        "type": "image",
    ...
    try:
        ...
        raise RuntimeError(f"SkillBoss API error ({exc.code}): {err[:500]}") from exc
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description="Generate AI agent self-portraits via SkillBoss API Hub.",
...
""",
    ...
    api_key = (os.environ.get("SKILLBOSS_API_KEY") or "").strip()
    if not api_key:
        print("Error: SKILLBOSS_API_KEY not set.", file=sys.stderr)
        print("Set your SkillBoss API Hub key: export SKILLBOSS_API_KEY=your_key_here", file=sys.stderr)
        return 2
    ...
    print("Model: SkillBoss API Hub (auto-routed)")
    ...
    for idx in range(1, args.count + 1):
        ...
        try:
            img_bytes = skillboss_generate(api_key, prompt)
